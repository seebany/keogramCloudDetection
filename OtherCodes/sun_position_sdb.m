function sun = sun_position_sdb(time, location)
...
% 4 Mar 2009    Downloaded version updated 2/23/09 and modified to accept
%               vector inputs. S. Datta-Barua.
...
topocentric_sun_position = topocentric_sun_position_calculate(...
    earth_heliocentric_position, location, observer_local_hour, ...
...
sun = sun_topocentric_zenith_angle_calculate(location, ...
    topocentric_sun_position, topocentric_local_hour);
...
A0 = repmat(L0_terms(:,1), 1, numel(JME));
B0 = repmat(L0_terms(:,2), 1, numel(JME));
C0 = L0_terms(:,3);

A1 = repmat(L1_terms(:,1), 1, numel(JME));
B1 = repmat(L1_terms(:,2), 1, numel(JME));
C1 = L1_terms(:,3);

A2 = repmat(L2_terms(:,1), 1, numel(JME));
B2 = repmat(L2_terms(:,2), 1, numel(JME));
C2 = L2_terms(:,3);

A3 = repmat(L3_terms(:,1), 1, numel(JME));
B3 = repmat(L3_terms(:,2), 1, numel(JME));
C3 = L3_terms(:,3);

A4 = repmat(L4_terms(:,1), 1, numel(JME));
B4 = repmat(L4_terms(:,2), 1, numel(JME));
C4 = L4_terms(:,3);

A5 = repmat(L5_terms(:,1), 1, numel(JME));
B5 = repmat(L5_terms(:,2), 1, numel(JME));
C5 = L5_terms(:,3);

% Compute the Earth Heliochentric longitude from the tabulated values. 
L0 = sum(A0 .* cos(B0 + (C0 * JME)),1);
L1 = sum(A1 .* cos(B1 + (C1 * JME)),1);
L2 = sum(A2 .* cos(B2 + (C2 * JME)),1);
L3 = sum(A3 .* cos(B3 + (C3 * JME)),1);
L4 = sum(A4 .* cos(B4 + (C4 * JME)),1);
L5 = A5 .* cos(B5 + (C5 * JME));


earth_heliocentric_position.longitude = (L0 + (L1 .* JME) + ...
    (L2 .* JME.^2) + (L3 .* JME.^3) + (L4 .* JME.^4) + (L5 .* JME.^5)) ...
    ./ 1e8; 
% Convert the longitude to degrees. 
earth_heliocentric_position.longitude = ...
    earth_heliocentric_position.longitude * 180/pi;
% Limit the range to [0,360[;
earth_heliocentric_position.longitude = ...
    set_to_range(earth_heliocentric_position.longitude, 0, 360)';
...
A0 = repmat(B0_terms(:,1), 1, numel(JME));
B0 = repmat(B0_terms(:,2), 1, numel(JME));
C0 = B0_terms(:,3);

A1 = repmat(B1_terms(:,1), 1, numel(JME));
B1 = repmat(B1_terms(:,2), 1, numel(JME));
C1 = B1_terms(:,3);
...
earth_heliocentric_position.latitude = ...
    earth_heliocentric_position.latitude * 180/pi;
% Limit the range to [0,360];
earth_heliocentric_position.latitude = ...
    set_to_range(earth_heliocentric_position.latitude, 0, 360)';
...
A0 = repmat(R0_terms(:,1), 1, numel(JME));
B0 = repmat(R0_terms(:,2), 1, numel(JME));
C0 = R0_terms(:,3);

A1 = repmat(R1_terms(:,1), 1, numel(JME));
B1 = repmat(R1_terms(:,2), 1, numel(JME));
C1 = R1_terms(:,3);

A2 = repmat(R2_terms(:,1), 1, numel(JME));
B2 = repmat(R2_terms(:,2), 1, numel(JME));
C2 = R2_terms(:,3);

A3 = repmat(R3_terms(:,1), 1, numel(JME));
B3 = repmat(R3_terms(:,2), 1, numel(JME));
C3 = R3_terms(:,3);

A4 = repmat(R4_terms(:,1), 1, numel(JME));
B4 = repmat(R4_terms(:,2), 1, numel(JME));
C4 = R4_terms(:,3);
...
% Units are in AU
earth_heliocentric_position.radius = (L0 + (L1 .* JME) + (L2 .* JME.^2)...
    + (L3 .* JME.^3) + (L4 .* JME.^4))' / 1e8; 
...
delta_longitude = ((repmat(nutation_terms(:,1),1,numel(JCE)) ...
    + (nutation_terms(:,2) * JCE))) ...
    .* sin(tabulated_argument);
delta_obliquity = ((repmat(nutation_terms(:,3),1,numel(JCE)) ...
    + (nutation_terms(:,4) * JCE))) ...
    .* cos(tabulated_argument);

% Nutation in longitude
nutation.longitude = sum(delta_longitude, 1)' / 36000000;

% Nutation in obliquity
nutation.obliquity = sum(delta_obliquity, 1)' / 36000000;
...
mean_obliquity = p(1)*U.^10 + p(2)*U.^9 + p(3)*U.^8 + p(4)*U.^7 + ...
    p(5)*U.^6 + p(6)*U.^5 + p(7)*U.^4 + p(8)*U.^3 + p(9)*U.^2 + ...
    p(10)*U + p(11);
...
function aberration_correction = abberation_correction_calculation(...
    earth_heliocentric_position)
...
aberration_correction = reshape(aberration_correction, ...
    numel(aberration_correction), 1);

function apparent_sun_longitude = apparent_sun_longitude_calculation(...
    sun_geocentric_position, nutation, aberration_correction)
...
function apparent_stime_at_greenwich ...
    = apparent_stime_at_greenwich_calculation(julian, nutation, ...
    true_obliquity)
...
function sun_rigth_ascension = sun_rigth_ascension_calculation(...
    apparent_sun_longitude, true_obliquity, sun_geocentric_position)
% This function compute the sun rigth ascension. 

argument_numerator = (sin(apparent_sun_longitude * pi/180) ...
    .* cos(true_obliquity * pi/180)) - ...
    (tan(sun_geocentric_position.latitude * pi/180) ...
    .* sin(true_obliquity * pi/180));
...
function sun_geocentric_declination ...
    = sun_geocentric_declination_calculation(apparent_sun_longitude, ...
    true_obliquity, sun_geocentric_position)

argument = (sin(sun_geocentric_position.latitude * pi/180) ...
    .* cos(true_obliquity * pi/180)) + ...
    (cos(sun_geocentric_position.latitude * pi/180) ...
    .* sin(true_obliquity * pi/180) ...
    .* sin(apparent_sun_longitude * pi/180));

sun_geocentric_declination = asin(argument) * 180/pi;

function observer_local_hour = observer_local_hour_calculation(...
    apparent_stime_at_greenwich, location, sun_rigth_ascension)
...
function topocentric_sun_position = topocentric_sun_position_calculate(...
    earth_heliocentric_position, location, observer_local_hour, ...
    sun_rigth_ascension, sun_geocentric_declination)
...
eq_horizontal_parallax = reshape(eq_horizontal_parallax, ...
    numel(eq_horizontal_parallax), 1);
...
x = cos(u) + ((location.altitude/6378140) ...
    .* cos(location.latitude * pi/180));

% Term y, used in the following calculations
y = (0.99664719 * sin(u)) + ((location.altitude/6378140) ...
    .* sin(location.latitude * pi/180));

% Parallax in the sun rigth ascension (in radians)
nominator = -x .* sin(eq_horizontal_parallax * pi/180) ...
    .* sin(observer_local_hour * pi/180);
denominator = cos(sun_geocentric_declination * pi/180) ...
    - (x .* sin(eq_horizontal_parallax * pi/180) ...
    .* cos(observer_local_hour * pi/180));
...
nominator = (sin(sun_geocentric_declination * pi/180) ...
    - (y.*sin(eq_horizontal_parallax * pi/180))) ...
    .* cos(sun_rigth_ascension_parallax);
denominator = cos(sun_geocentric_declination * pi/180) ...
    - (x.*sin(eq_horizontal_parallax * pi/180)) ...
    .* cos(observer_local_hour * pi/180);
topocentric_sun_position.declination ...
    = atan2(nominator, denominator) * 180/pi;
...
argument = (sin(location.latitude * pi/180) ...
    .* sin(topocentric_sun_position.declination * pi/180)) + ...
    (cos(location.latitude * pi/180) ...
    .* cos(topocentric_sun_position.declination * pi/180) ...
    .* cos(topocentric_local_hour * pi/180));
...
denominator = (cos(topocentric_local_hour * pi/180) ...
    .* sin(location.latitude * pi/180)) - ...
    (tan(topocentric_sun_position.declination * pi/180) ...
    .* cos(location.latitude * pi/180));
